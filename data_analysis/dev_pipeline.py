...
# T1 = (datetime.datetime.strptime(end_date, '%Y-%m-%d') + datetime.timedelta(days=1)).strftime('%Y-%m-%d') 
...
for i in range(len(gauge_locs)):
    ...
    ## glofas buffer search 
    glofas_buffer = dev_utils.iterative_pixel_search(glofas_time,
                                                      loc,
                                                      init_x = coords_4326[i][0], 
                                                      init_y = coords_4326[i][1], 
                                                      x_tol = cell_size_glofas, 
                                                      y_tol = cell_size_glofas, 
                                                      n_iter = n_iter, 
                                                      cols = ['dis24', 'upArea'],
                                                      coords = ['time'],
                                                      locs = ['lon', 'lat'])
    
    collect_glofas = collect_glofas.append(glofas_buffer)
...
glofas_set = collect_glofas[ collect_glofas['match_gauge'].isin(subset_locations)]
...
# show_glofas_search = dev_plotter.dashboard(glofas_time, glofas_set, gauge_set, subset_locations,
#                                             model = 'glofas', coords = ['lon', 'lat'],
#                                             cell_size = 0.1)
...
## total set 
# show_efas_search = dev_plotter.dashboard(efas_time, collect_efas, gauge_time, gauge_locs)
